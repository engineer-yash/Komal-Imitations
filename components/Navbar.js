  ...
  return (
    <motion.nav
      ...
      className={`sticky top-0 z-50 w-full transition-all duration-500 ${
        scrolled 
          ? 'bg-white/98 backdrop-blur-md shadow-lg border-b border-primary/20' 
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex items-center transition-all duration-500 ${
          ...
          {/* Left Navigation - Hidden on mobile when not scrolled */}
              ...
              <>
                {navLinks.slice(0, 2).map((link) => (
                  ...
                  >
                    <Link
                      href={link.href}
                      className="text-sm font-medium text-gray-700 hover:text-primary transition-colors duration-300 relative group"
          ...
          <Link href="/" className={`flex items-center justify-center transition-all duration-500 ${
            scrolled ? 'flex-shrink-0' : 'flex-1'
          }`}>
            <AnimatePresence mode="wait">
              {scrolled ? (
                // K Logo on scroll (left aligned)
                <motion.div
                  key="k-logo"
          ...
          <div className={`hidden lg:flex items-center space-x-8 flex-1 ${
            ...
            {navLinks.slice(scrolled ? 0 : 2).map((link, index) => (
              <motion.div
                key={link.name}
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
